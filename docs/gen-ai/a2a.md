...
This refinement applies to A2A `SendMessage` and
`SendStreamingMessage` operations, recorded as `send_message` and
`send_streaming_message` in `a2a.method.name`. `GetTask` only retrieves
task state and MUST NOT use this refinement.
